...
export default class JingleSessionPC extends JingleSession {
    ...
    _renegotiate(optionalRemoteSdp) {
        const defaultRemoteSdp = this.peerconnection.remoteDescription.sdp;

        // FIXME new SDP() may not be necessary - raw SDP is needed anyway
        const remoteSdp
            = optionalRemoteSdp
                || (defaultRemoteSdp && new SDP(defaultRemoteSdp));
    ...
    replaceTrack(oldTrack, newTrack) {
        const workFunction = finishedCallback => {
            ...
            // FIXME it makes sense to check for ACTIVE state in other places,
            // because it's probably the only state when _renegotiate makes
            // sense (both local and remote SDPs are available).
